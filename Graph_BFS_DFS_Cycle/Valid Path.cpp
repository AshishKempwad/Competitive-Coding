...
void dfs(int x,int y, vector<vector<bool>>&vis,int target_x,int target_y,int no_of_circles,int radius,vector<int>&x_coordinates,vector<int>&y_coordinates)
{
    ...
    for(int i=0;i<8;i++)
    {
        ...
        if(newx >= 0 && newx < target_x && newy >=0 && newy < target_y && vis[newx][newy]==false && check_in_circle(newx,newy,no_of_circles,radius,x_coordinates,y_coordinates)==false)
        {
...
string Solution::solve(int A, int B, int C, int D, vector<int> &E, vector<int> &F)
{
    ...
    if(check_in_circle(0,0,C,D,E,F)==1)  //if we are inside the circle then, we fail as we enetered circle 
